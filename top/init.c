...
extern const struct nusa_init_struct __start_nusa_init[] __WEAK;
extern const struct nusa_init_struct __stop_nusa_init[] __WEAK;

void nusa_init_level(enum nusa_init_flags required_flag, uint16_t start_level, uint16_t stop_level) {
    ...
    const struct nusa_init_struct *last = NULL;
    for (;;) {
        /* search for the lowest uncalled hook to call */
        LTRACEF("last %p, last_called_level %#x\n", last, last_called_level);

        const struct nusa_init_struct *found = NULL;
        bool seen_last = false;
        for (const struct nusa_init_struct *ptr = __start_nusa_init; ptr != __stop_nusa_init; ptr++) {
