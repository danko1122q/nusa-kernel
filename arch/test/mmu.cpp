...
bool map_user_pages() {
    ...
    if (arch_mmu_supports_user_aspaces()) {
        ...
        auto aspace_cleanup = nusa::make_auto_call([&]() { arch_mmu_destroy_aspace(&as); });
        ...
        auto pages_cleanup = nusa::make_auto_call([&]() { pmm_free(&pages); });
...
bool context_switch() {
    ...
    if (arch_mmu_supports_user_aspaces()) {
        ...
        auto cleanup_switch = nusa::make_auto_call([&]() { arch_mmu_context_switch(NULL); });

        // map a page, verify can be read through the page
        vm_page_t *p = pmm_alloc_page();
        ASSERT_NONNULL(p, "page");
        auto page_cleanup = nusa::make_auto_call([&]() { pmm_free_page(p); });
