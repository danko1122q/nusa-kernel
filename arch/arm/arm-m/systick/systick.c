...
static nusa_time_t tick_interval_ms;
static nusa_bigtime_t tick_interval_us;
...
static void arm_cm_systick_set_periodic(nusa_time_t period) {
    LTRACEF("cnusa_freq %u, period %u\n", tick_rate, (uint)period);
...
status_t platform_set_periodic_timer(platform_timer_callback callback, void *arg, nusa_time_t interval) {
...
nusa_time_t current_time(void) {
    ...
    nusa_time_t res = (ticks * tick_interval_ms) + delta;
...
nusa_bigtime_t current_time_hires(void) {
    ...
    nusa_bigtime_t res = (ticks * tick_interval_us) + delta;
