...
ifeq ($(ARCH_x86_TOOLCHAIN_PREFIX),)
# Using native gcc, set a dummy value to pass the check
TOOLCHAIN_PREFIX := native-
else
...
ifeq ($(SUBARCH),x86-64)
ifndef TOOLCHAIN_PREFIX
ifeq ($(ARCH_x86_64_TOOLCHAIN_PREFIX),)
# Using native gcc, set a dummy value to pass the check
TOOLCHAIN_PREFIX := native-
else
...
# Add -m32 for native gcc on x86-32
ifeq ($(SUBARCH),x86-32)
ifeq ($(TOOLCHAIN_PREFIX),native-)
ARCH_COMPILEFLAGS += -m32
ARCH_LDFLAGS += -m elf_i386
GLOBAL_LDFLAGS += -m elf_i386
GLOBAL_MODULE_LDFLAGS += -m elf_i386
endif
endif
...
# Get libgcc, handle native gcc specially
ifeq ($(TOOLCHAIN_PREFIX),native-)
LIBGCC := $(shell gcc $(GLOBAL_COMPILEFLAGS) $(ARCH_COMPILEFLAGS) -print-libgcc-file-name)
else
# Get libgcc, handle native gcc specially
ifeq ($(TOOLCHAIN_PREFIX),native-)
LIBGCC := $(shell gcc $(GLOBAL_COMPILEFLAGS) $(ARCH_COMPILEFLAGS) -print-libgcc-file-name)
