...
enum class wanusa_cb_ret_op {
    HALT,
    RESTART,
    ALLOC_PT
};

struct wanusa_cb_ret {
    static wanusa_cb_ret OpHalt(int err) { return { wanusa_cb_ret_op::HALT, err, false, 0, false }; }
    static wanusa_cb_ret OpRestart() { return { wanusa_cb_ret_op::RESTART, NO_ERROR, false, 0, false }; }
    static wanusa_cb_ret OpCommitHalt(riscv_pte_t pte, bool unmap, int err) { return { wanusa_cb_ret_op::HALT, err, true, pte, unmap }; }
    static wanusa_cb_ret OpCommitRestart(riscv_pte_t pte, bool unmap) { return { wanusa_cb_ret_op::RESTART, NO_ERROR, true, pte, unmap }; }
    static wanusa_cb_ret OpAllocPT() { return { wanusa_cb_ret_op::ALLOC_PT, 0, false, 0, false }; }
...
using page_wanusa_cb = wanusa_cb_ret(*)(uint level, uint index, riscv_pte_t pte, vaddr_t *vaddr);
...
restart:
    ...
    for (;;) {
        ...
        if ((pte & RISCV_PTE_V) && !(pte & RISCV_PTE_PERM_MASK)) {
            ...
            switch (ret.op) {
                case wanusa_cb_ret_op::HALT:
                case wanusa_cb_ret_op::RESTART:
                    ...
                    if (ret.op == wanusa_cb_ret_op::HALT) {
                        // stop here
                        return ret.err;
                    } else { // RESTART
                ...
                case wanusa_cb_ret_op::ALLOC_PT:
...
int arch_mmu_map(arch_aspace_t *aspace, const vaddr_t _vaddr, paddr_t paddr, uint count, const uint flags) {
    ...
    auto map_cb = [&paddr, &count, aspace, flags](uint level, uint index, riscv_pte_t pte, vaddr_t *vaddr) -> wanusa_cb_ret {
        ...
        if ((pte & RISCV_PTE_V)) {
            ...
            return wanusa_cb_ret::OpHalt(ERR_ALREADY_EXISTS);
        }
        ...
        if (level > 0) {
            ...
            return wanusa_cb_ret::OpAllocPT();
        }
        ...
        *vaddr += PAGE_SIZE;
        ...
        if (count == 0) {
            return wanusa_cb_ret::OpCommitHalt(temp_pte, false, NO_ERROR);
        } else {
            return wanusa_cb_ret::OpCommitRestart(temp_pte, false);
        }
    };
...
status_t arch_mmu_query(arch_aspace_t *aspace, const vaddr_t _vaddr, paddr_t *paddr, uint *flags) {
    ...
    auto query_cb = [paddr, flags](uint level, uint index, riscv_pte_t pte, vaddr_t *vaddr) -> wanusa_cb_ret {
        ...
        if (pte & RISCV_PTE_V) {
            ...
            return wanusa_cb_ret::OpHalt(NO_ERROR);
        } else {
            // couldnt find our page, stop
            return wanusa_cb_ret::OpHalt(ERR_NOT_FOUND);
        }
    };
...
int arch_mmu_unmap(arch_aspace_t *aspace, const vaddr_t _vaddr, const uint _count) {
    ...
    auto count = _count;
    auto unmap_cb = [&count]
        (uint level, uint index, riscv_pte_t pte, vaddr_t *vaddr) -> wanusa_cb_ret {
        LTRACEF("level %u, index %u, pte %#lx, vaddr %#lx\n", level, index, pte, *vaddr);
        ...
        if (pte & RISCV_PTE_V) {
            ...
            *vaddr += PAGE_SIZE;
            count--;
            if (count == 0) {
                return wanusa_cb_ret::OpCommitHalt(0, true, NO_ERROR);
            } else {
                return wanusa_cb_ret::OpCommitRestart(0, true);
            }
        } else {
            ...
            *vaddr += PAGE_SIZE;
            ...
            if (count == 0) {
                ...
                return wanusa_cb_ret::OpRestart();
            }
        }
    };
