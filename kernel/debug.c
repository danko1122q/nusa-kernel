...
static enum handler_return threadload(struct timer *t, nusa_time_t now, void *arg) {
    static struct thread_stats old_stats[SMP_MAX_CPUS];
    static nusa_bigtime_t last_idle_time[SMP_MAX_CPUS];
    ...
    for (uint i = 0; i < SMP_MAX_CPUS; i++) {
        ...
        nusa_bigtime_t idle_time = thread_stats[i].idle_time;
        ...
        nusa_bigtime_t delta_time = idle_time - last_idle_time[i];
        nusa_bigtime_t busy_time = 1000000ULL - (delta_time > 1000000ULL ? 1000000ULL : delta_time);
