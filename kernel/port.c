...
static inline status_t read_no_lock(read_port_t *rp, nusa_time_t timeout, port_result_t *result) {
...
status_t port_read(port_t port, nusa_time_t timeout, port_result_t *result) {
    if (!port || !result)
        return ERR_INVALID_ARGS;
