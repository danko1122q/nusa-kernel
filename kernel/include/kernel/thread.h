...
struct thread_specific_stats {
    nusa_bigtime_t total_run_time;
    nusa_bigtime_t last_run_timestamp;
...
struct timer;
enum handler_return thread_timer_tick(struct timer *, nusa_time_t now, void *arg);
...
#if THREAD_STATS
struct thread_stats {
    nusa_bigtime_t idle_time;
    nusa_bigtime_t last_idle_timestamp;
    ulong reschedules;
