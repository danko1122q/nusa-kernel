...
static enum handler_return timer_tick(void *arg, nusa_time_t now);
...
static void timer_set(timer_t *timer, nusa_time_t delay, nusa_time_t period, timer_callback callback, void *arg) {
    nusa_time_t now;
 ...
 *   enum handler_return callback(struct timer *, nusa_time_t now, void *arg) { ... }
 */
void timer_set_oneshot(timer_t *timer, nusa_time_t delay, timer_callback callback, void *arg) {
    if (delay == 0)
        delay = 1;
 ...
 *   enum handler_return callback(struct timer *, nusa_time_t now, void *arg) { ... }
 */
void timer_set_periodic(timer_t *timer, nusa_time_t period, timer_callback callback, void *arg) {
    if (period == 0)
        period = 1;
...
//  KEVLOG_TIMER_TICK(); // enable only if necessary
    ...
    if (timer) {
        ...
        nusa_time_t delay = timer->scheduled_time - now;
