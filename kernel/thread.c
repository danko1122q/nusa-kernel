...
status_t thread_join(thread_t *t, int *retcode, nusa_time_t timeout) {
    DEBUG_ASSERT(t->magic == THREAD_MAGIC);
...
void thread_resched(void) {
    ...
    nusa_bigtime_t now = current_time_hires();
...
enum handler_return thread_timer_tick(struct timer *t, nusa_time_t now, void *arg) {
    thread_t *current_thread = get_current_thread();
...
static enum handler_return thread_sleep_handler(timer_t *timer, nusa_time_t now, void *arg) {
 ...
 */
void thread_sleep(nusa_time_t delay) {
    timer_t timer;
 ...
 *
 * This function is called once, from nusa_main()
 */
...
static enum handler_return wait_queue_timeout_handler(timer_t *timer, nusa_time_t now, void *arg) {
 ...
 */
status_t wait_queue_block(wait_queue_t *wait, nusa_time_t timeout) {
    timer_t timer;
