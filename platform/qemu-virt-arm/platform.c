...
void platform_early_init(void) {
    ...
    fdtwanusa_setup_memory(fdt, MEMORY_BASE_PHYS, MEMORY_BASE_PHYS, DEFAULT_MEMORY_SIZE);
}
...
void platform_postvm_init(uint level) {
    ...
    if (fdtwanusa_setup_gic(fdt) != NO_ERROR) {
...
void platform_init(void) {
    ...
    fdtwanusa_setup_cpus_arm(fdt);

    /* configure and start pci from device tree */
    status_t err = fdtwanusa_setup_pci(fdt);
    if (err >= NO_ERROR) {
