...
nusa_time_t pit_current_time(void) {
    spin_lock_saved_state_t state;
    spin_lock_irqsave(&lock, state);

    nusa_time_t time = (nusa_time_t) (timer_current_time >> 32);
...
nusa_bigtime_t pit_current_time_hires(void) {
    spin_lock_saved_state_t state;
    spin_lock_irqsave(&lock, state);

    nusa_bigtime_t time = (nusa_bigtime_t) ((timer_current_time >> 22) * 1000) >> 10;
...
status_t pit_set_periodic_timer(platform_timer_callback callback, void *arg, nusa_time_t interval) {
...
status_t pit_set_oneshot_timer(platform_timer_callback callback, void *arg, nusa_time_t interval) {
