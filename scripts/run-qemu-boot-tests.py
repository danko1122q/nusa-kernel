...
class QEMUTestRunner:
    def __init__(self, nusa_root):
        self.nusa_root = Path(nusa_root)
        self.architectures = {
            'arm': {
                'script': 'do-qemuarm',
                'args': '',
    ...
    def run_qemu_test(self, arch, arch_config, quiet=False, log_dir=None):
        ...
        script_path = self.nusa_root / 'scripts' / arch_config['script']
        ...
        try:
            process = subprocess.Popen(
                ...
                env={**os.environ, 'LK_ROOT': str(self.nusa_root), 'RUN_UNITTESTS_AT_BOOT': '1'},
            )
...
def main():
    parser = argparse.ArgumentParser(description='Run NUSA QEMU tests for multiple architectures')
    ...
    parser.add_argument('--nusa-root', default='.',
                       help='Path to NUSA root directory (default: current directory)')
    ...
    # Change to NUSA root directory
    nusa_root = os.path.abspath(args.nusa_root)
    if not os.path.exists(os.path.join(nusa_root, 'makefile')) and not os.path.exists(os.path.join(nusa_root, 'Makefile')):
        print(f"Error: {nusa_root} doesn't appear to be the NUSA root directory")
        return 1

    runner = QEMUTestRunner(nusa_root)
