...
ifndef NUSAROOT
$(error please define NUSAROOT to the root of the nusa build system)
endif
...
ifneq ($(project-name),)
ifneq ($(strip $(foreach d,$(NUSAINC),$(wildcard $(d)/project/$(project-name).mk))),)
do-nothing := 1
...
OUTBIN := $(BUILDDIR)/nusa.bin
OUTELF := $(BUILDDIR)/nusa.elf
CONFIGHEADER := $(BUILDDIR)/config.h

GLOBAL_INCLUDES := $(BUILDDIR) $(addsuffix /include,$(NUSAINC))
...
GLOBAL_LDFLAGS += $(addprefix -L,$(NUSAINC))
...
ifneq ($(wildcard project/target/$(TARGET).mk),)
include project/target/$(TARGET).mk
endif
ifndef PLATFORM
...
CCACHE ?=
# Handle native gcc (when TOOLCHAIN_PREFIX is "native-")
ifeq ($(TOOLCHAIN_PREFIX),native-)
CC ?= $(CCACHE) gcc
LD ?= ld
OBJDUMP ?= objdump
OBJCOPY ?= objcopy
