...
static void tcp_timer_set(tcp_socket_t *s, net_timer_t *timer, net_timer_callback_t cb, nusa_time_t delay) {
    DEBUG_ASSERT(s);
    DEBUG_ASSERT(timer);
...
status_t tcp_connect(tcp_socket_t **handle, uint32_t addr, uint16_t port) {
    ...
    nusa_bigtime_t t = current_time_hires();
...
status_t tcp_accept_timeout(tcp_socket_t *listen_socket, tcp_socket_t **accept_socket, nusa_time_t timeout) {
