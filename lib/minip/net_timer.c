...
bool net_timer_set(net_timer_t *t, net_timer_callback_t cb, void *callback_args, nusa_time_t delay) {
...
static nusa_time_t net_timer_work_routine(void) {
    nusa_time_t now = current_time();
    nusa_time_t delay = INFINITE_TIME;
...
static int net_timer_work_thread(void *args) {
    for (;;) {
        ...
        nusa_time_t delay = net_timer_work_routine();
        if (delay != INFINITE_TIME) {
