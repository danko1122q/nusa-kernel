...
status_t watchdog_init(watchdog_t *dog, nusa_time_t timeout, const char *name);
 ...
 * automatically dismiss a platform's HW watchdog using NUSA timers.  Platforms
 * must supply
 *
 ...
 * enable the HW watchdog.  SW watchdogs are based on NUSA timers, and should be
 ...
 * something managed to break timers on NUSA.
 */

extern status_t platform_watchdog_init(nusa_time_t  target_timeout,
                                       nusa_time_t *recommended_pet_period);
extern void platform_watchdog_set_enabled(bool enabled);
extern void platform_watchdog_pet(void);

status_t watchdog_hw_init(nusa_time_t timeout);
