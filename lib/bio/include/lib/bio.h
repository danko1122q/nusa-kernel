...
//
// This header declares the NUSA block device abstraction. A block device (bdev_t)
...
// - Many functions return ssize_t/status_t; on error they return a negative NUSA
...
typedef struct bdev {
    ...
    // negative NUSA error code on failure.
