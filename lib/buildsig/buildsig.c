...
status_t buildsig_search(const void *_ptr, size_t search_len, size_t max_len, const nusa_version_t **version) {
    if (max_len < search_len)
        return ERR_INVALID_ARGS;

    if (max_len < sizeof(nusa_version_t))
        return ERR_INVALID_ARGS;
    ...
    for (size_t pos = 0; pos < search_len / 4; pos++) {
        ...
        if ((size_t)sig->version - (size_t)ptr > max_len - sizeof(nusa_version_t))
            continue;
