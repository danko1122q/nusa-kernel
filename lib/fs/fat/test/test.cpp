...
bool test_fat_dir_root() {
    ...
    auto unmount_cleanup = nusa::make_auto_call([]() { fs_unmount(test_path); });
    ...
    auto closedir_cleanup = nusa::make_auto_call([&]() { fs_close_dir(handle); });
...
bool test_file_read(const char *path, const unsigned char *test_file_buffer, size_t test_file_size) {
    ...
    auto closefile_cleanup = nusa::make_auto_call([&]() { fs_close_file(handle); });

    ASSERT_NONNULL(handle);

    const size_t buflen = test_file_size * 2; // should be somewhat larger than test_file_size
    char *buf = new char[buflen];
    auto delete_buffer = nusa::make_auto_call([&]() { delete[] buf; });
...
bool test_fat_multi_open() {
    ...
    // open a file three times simultaneously
    {
        ...
        auto closefile_cleanup1 = nusa::make_auto_call([&]() { fs_close_file(handle1); });

        filehandle *handle2 = nullptr;
        ASSERT_EQ(NO_ERROR, fs_open_file(test_path "/hello.txt", &handle2));
        auto closefile_cleanup2 = nusa::make_auto_call([&]() { fs_close_file(handle2); });
    ...
    // open a dir three times simultaneously
    {
        ...
        auto closedir_cleanup1 = nusa::make_auto_call([&]() { fs_close_dir(handle1); });

        dirhandle *handle2 = nullptr;
        ASSERT_EQ(NO_ERROR, fs_open_dir(test_path "/dir.a", &handle2));
        auto closedir_cleanup2 = nusa::make_auto_call([&]() { fs_close_dir(handle2); });
