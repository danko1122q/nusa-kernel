...
status_t fdtwanusa_reserve_fdt_memory(const void *fdt, paddr_t fdt_phys) {
...
status_t fdtwanusa_setup_memory(const void *fdt, paddr_t fdt_phys, paddr_t default_mem_base, size_t default_mem_size) {
#if WITH_KERNEL_VM
    // TODO: consider having pmm make a copy so that this doesn't have to be static
    static pmm_arena_t arenas[8];
#endif

    struct fdt_wanusa_memory_region mem[8];
    struct fdt_wanusa_memory_region reserved_mem[16];
    size_t mem_count = countof(mem);
    size_t reserved_mem_count = countof(reserved_mem);

    // find memory from the FDT
    status_t err = fdt_wanusa_find_memory(fdt, mem, &mem_count, reserved_mem, &reserved_mem_count);
    ...
    fdtwanusa_reserve_fdt_memory(fdt, fdt_phys);

    return NO_ERROR;
}

#if ARCH_RISCV
status_t fdtwanusa_setup_cpus_riscv(const void *fdt) {
#if WITH_SMP
    struct fdt_wanusa_cpu_info cpus[SMP_MAX_CPUS];
    size_t cpu_count = countof(cpus);

    status_t err = fdt_wanusa_find_cpus(fdt, cpus, &cpu_count);
    if (err >= NO_ERROR) {
...
#if ARCH_ARM || ARCH_ARM64
status_t fdtwanusa_setup_cpus_arm(const void *fdt) {
    ...
    err = fdt_wanusa_find_cpus(fdt, cpus, &cpu_count);
    if (err >= NO_ERROR) {
        if (cpu_count > 0) {
            dprintf(INFO, "FDT: found %zu cpu%c\n", cpu_count, cpu_count == 1 ? ' ' : 's');
            ...
            nusa_init_secondary_cpus(cpu_count - 1);
...
#if WITH_DEV_BUS_PCI
status_t fdtwanusa_setup_pci(const void *fdt) {
    /* detect pci */
    struct fdt_wanusa_pcie_info pcie_info[4] = {};

    size_t count = countof(pcie_info);
    status_t err = fdt_wanusa_find_pcie_info(fdt, pcie_info, &count);
    LTRACEF("fdt_wanusa_find_pcie_info returns %d, count %zu\n", err, count);
    if (err == NO_ERROR) {
...
status_t fdtwanusa_setup_gic(const void *fdt) {
    struct fdt_wanusa_gic_info gic_info[4] = {};

    size_t count = countof(gic_info);
    status_t err = fdt_wanusa_find_gic_info(fdt, gic_info, &count);

    LTRACEF("fdt_wanusa_find_gic_info returns %d, count %zu\n", err, count);
