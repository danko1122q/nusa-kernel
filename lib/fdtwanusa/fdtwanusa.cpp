...
status_t fdt_wanusa_dump(const void *fdt) {
    auto cb = [](const fdt_wanusa_state &state, const char *name) {
...
status_t fdt_wanusa_find_cpus(const void *fdt, struct fdt_wanusa_cpu_info *cpu, size_t *cpu_count) {
    ...
    *cpu_count = 0;
    ...
    auto walker = [max_cpu_count, cpu, cpu_count](const fdt_wanusa_state &state, const char *name) {
...
status_t fdt_wanusa_find_memory(const void *fdt, struct fdt_wanusa_memory_region *memory, size_t *mem_count,
    ...
    *mem_count = *reserved_mem_count = 0;
    ...
    auto walker = [&](const fdt_wanusa_state &state, const char *name) {
        int err;
...
status_t fdt_wanusa_find_pcie_info(const void *fdt, struct fdt_wanusa_pcie_info *info, size_t *count) {
    size_t info_len = *count;
    *count = 0;
    auto walker = [info, info_len, &count](const fdt_wanusa_state &state, const char *name) {
...
status_t fdt_wanusa_find_gic_info(const void *fdt, struct fdt_wanusa_gic_info *info, size_t *count) {
    const size_t info_len = *count;
    *count = 0;
    auto walker = [info, info_len, &count](const fdt_wanusa_state &state, const char *name) {
        /* look for a gic node and pass the address of the ecam and other info to the callback */
        fdt_wanusa_gic_info *infop = &info[*count];
        if (*count < info_len) {
            int lenp;
