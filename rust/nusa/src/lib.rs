...
pub extern crate nusa_sys as sys;
...
/// Return the current stderr from nusa's simple stdio implementation.
#[inline(always)]
...
/// Return the current stderr from nusa's simple stdio implementation.
#[inline(always)]
pub fn nusa_stderr() -> *mut sys::FILE {
    get_stdfile(2)
}

/// Return the current stdout from nusa's simple stdio implementation.
#[inline(always)]
pub fn nusa_stdout() -> *mut sys::FILE {
    get_stdfile(1)
}

/// Return the current stdin from nusa's simple stdio implementation.
#[inline(always)]
pub fn nusa_stdin() -> *mut sys::FILE {
    get_stdfile(0)
}

extern "C" fn sample_rust_hook(_level: c_uint) {
    unsafe {
        sys::fputs(c"*** INIT: nusa init\n".as_ptr(), nusa_stderr());
    }
    ::log::warn!("Warning: Message here");
}

/*
#[unsafe(link_section = "nusa_init")]
#[used]
static SETUP_RUST_HOOK: sys::nusa_init_struct = sys::nusa_init_struct {
    level: init::nusa_init_level::LK_INIT_LEVEL_PLATFORM.0 as u16,
    flags: init::nusa_init_flags::LK_INIT_FLAG_PRIMARY_CPU.0 as u16,
    hook: Some(sample_rust_hook),
    name: c"nusa_rust_hook".as_ptr(),
};
*/
LK_INIT_HOOK!(
    SETUP_RUST_HOOK,
    sample_rust_hook,
    init::nusa_init_level::LK_INIT_LEVEL_PLATFORM
);
...
fn handle_panic(info: &PanicInfo) -> ! {
    ...
    unsafe { sys::panic(c"Rust in NUSA %s\n".as_ptr(), panic_message_c.as_ptr()) }
}
