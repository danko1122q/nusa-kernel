...
//!
//! NUSA commonly uses an initialization pattern where a value is declared
...
//!
//! In order to handle this from Rust, we provide this NusaOnce type, which wraps
...
//! in-place, which is needed for many things in NUSA, as the NUSA list type uses
...
unsafe impl<T: Sync> Sync for NusaOnce<T> {}
...
impl<T> NusaOnce<T> {
    ...
    pub fn early_init(&self, init: impl FnOnce(*mut T)) {
        ...
        if prev_state == States::Uninit as u8 {
            ...
            panic!("NusaOnce early_init called out of order");
        }
    }
    ...
    pub unsafe fn init(&self, init: impl FnOnce(*mut T)) {
        ...
        if prev_state == States::Uninit as u8 || prev_state == States::EarlyInitted as u8 {
            ...
            panic!("NusaOnce init called out of order");
        }
    }
    ...
    pub fn get(&self) -> &T {
        ...
        if state == States::Initialized as u8 {
            ...
            panic!("NusaOnce get called before initialized");
        }
    }
    ...
    pub fn get_early(&self) -> &T {
        ...
        if state == States::Initialized as u8 || state == States::EarlyInitted as u8 {
            ...
            panic!("NusaOnce get_early called before early initialized");
        }
    }
}
