...
static status_t usb_register_cb(
    ...
    if (op == USB_CB_ONLINE) {
        for (int i = 0; i < MAX_USB_ENDPOINT_PAIRS; i++) {
            if ((0x1 << i) & chan->registered_bunusa_eps_in) {
                usbc_setup_endpoint(i, USB_IN, 0x40, USB_BULK);
            }
            if ((0x1 << i) & chan->registered_bunusa_eps_out) {
...
void cdcserial_create_channel(cdcserial_channel_t *chan, int data_ep_addr, int ctrl_ep_addr) {
    ...
    chan->registered_bunusa_eps_in = 0;
    chan->registered_bunusa_eps_out = 0;
    ...
    chan->registered_bunusa_eps_in |= (0x1 << data_ep_addr);
    chan->registered_bunusa_eps_out |= (0x1 << data_ep_addr);
