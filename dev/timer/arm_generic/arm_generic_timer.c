...
static uint64_t nusa_time_to_cntpct(nusa_time_t nusa_time) {
    return u64_mul_u32_fp32_64(nusa_time, cntpct_per_ms);
}

static nusa_time_t cntpct_to_nusa_time(uint64_t cntpct) {
    return u32_mul_u64_fp32_64(cntpct, ms_per_cntpct);
}

static nusa_bigtime_t cntpct_to_nusa_bigtime(uint64_t cntpct) {
...
status_t platform_set_oneshot_timer(platform_timer_callback callback, void *arg, nusa_time_t interval) {
    uint64_t cntpct_interval = nusa_time_to_cntpct(interval);
...
nusa_bigtime_t current_time_hires(void) {
    return cntpct_to_nusa_bigtime(read_cntpct());
}

nusa_time_t current_time(void) {
    return cntpct_to_nusa_time(read_cntpct());
}
...
static void test_nusa_time_to_cntpct(uint32_t cntfrq, nusa_time_t nusa_time) {
    uint64_t cntpct = nusa_time_to_cntpct(nusa_time);
    uint64_t expected_cntpct = ((uint64_t)cntfrq * nusa_time + 500) / 1000;

    test_time_conversion_check_result(cntpct, expected_cntpct, 1, false);
    LTRACEF_LEVEL(2, "nusa_time_to_cntpct(%u): got %llu, expect %llu\n", nusa_time, cntpct, expected_cntpct);
}

static void test_cntpct_to_nusa_time(uint32_t cntfrq, nusa_time_t expected_nusa_time, uint32_t wrap_count) {
    nusa_time_t nusa_time;
    uint64_t cntpct;

    cntpct = (uint64_t)cntfrq * expected_nusa_time / 1000;
    if ((uint64_t)cntfrq * wrap_count > UINT_MAX) {
        cntpct += (((uint64_t)cntfrq << 32) / 1000) * wrap_count;
    } else {
        cntpct += (((uint64_t)(cntfrq * wrap_count) << 32) / 1000);
    }
    nusa_time = cntpct_to_nusa_time(cntpct);

    test_time_conversion_check_result(nusa_time, expected_nusa_time, (1000 + cntfrq - 1) / cntfrq, true);
    LTRACEF_LEVEL(2, "cntpct_to_nusa_time(%llu): got %u, expect %u\n", cntpct, nusa_time, expected_nusa_time);
}

static void test_cntpct_to_nusa_bigtime(uint32_t cntfrq, uint64_t expected_s) {
    nusa_bigtime_t expected_nusa_bigtime = expected_s * 1000 * 1000;
    uint64_t cntpct = (uint64_t)cntfrq * expected_s;
    nusa_bigtime_t nusa_bigtime = cntpct_to_nusa_bigtime(cntpct);

    test_time_conversion_check_result(nusa_bigtime, expected_nusa_bigtime, (1000 * 1000 + cntfrq - 1) / cntfrq, false);
    LTRACEF_LEVEL(2, "cntpct_to_nusa_bigtime(%llu): got %llu, expect %llu\n", cntpct, nusa_bigtime, expected_nusa_bigtime);
}

static void test_time_conversions(uint32_t cntfrq) {
    test_nusa_time_to_cntpct(cntfrq, 0);
    test_nusa_time_to_cntpct(cntfrq, 1);
    test_nusa_time_to_cntpct(cntfrq, INT_MAX);
    test_nusa_time_to_cntpct(cntfrq, INT_MAX + 1U);
    test_nusa_time_to_cntpct(cntfrq, ~0);
    test_cntpct_to_nusa_time(cntfrq, 0, 0);
    test_cntpct_to_nusa_time(cntfrq, INT_MAX, 0);
    test_cntpct_to_nusa_time(cntfrq, INT_MAX + 1U, 0);
    test_cntpct_to_nusa_time(cntfrq, ~0, 0);
    test_cntpct_to_nusa_time(cntfrq, 0, 1);
    test_cntpct_to_nusa_time(cntfrq, 0, 7);
    test_cntpct_to_nusa_time(cntfrq, 0, 70);
    test_cntpct_to_nusa_time(cntfrq, 0, 700);
    test_cntpct_to_nusa_bigtime(cntfrq, 0);
    test_cntpct_to_nusa_bigtime(cntfrq, 1);
    test_cntpct_to_nusa_bigtime(cntfrq, 60 * 60 * 24);
    test_cntpct_to_nusa_bigtime(cntfrq, 60 * 60 * 24 * 365);
    test_cntpct_to_nusa_bigtime(cntfrq, 60 * 60 * 24 * (365 * 10 + 2));
    test_cntpct_to_nusa_bigtime(cntfrq, 60ULL * 60 * 24 * (365 * 100 + 2));
}
