...
struct virtio_bnusa_config {
    uint64_t capacity;
    uint32_t size_max;
    uint32_t seg_max;
    struct virtio_bnusa_geometry {
        uint16_t cylinders;
        uint8_t heads;
        uint8_t sectors;
    } geometry;
    uint32_t bnusa_size;
    struct virtio_bnusa_topology {
    ...
    struct virtio_bnusa_zoned_characteristics {
...
STATIC_ASSERT(sizeof(struct virtio_bnusa_config) == 96);

struct virtio_bnusa_req {
    uint32_t type;
    uint32_t ioprio; // v1.3 says this is 'reserved'
    uint64_t sector;
};
STATIC_ASSERT(sizeof(struct virtio_bnusa_req) == 16);

struct virtio_bnusa_discard_write_zeroes {
    uint64_t sector;
...
status_t virtio_block_init(struct virtio_device *dev, uint32_t host_features) {
    ...
    volatile struct virtio_bnusa_config *config = (struct virtio_bnusa_config *)dev->config_ptr;

    LTRACEF("capacity %" PRIx64 "\n", config->capacity);
    LTRACEF("size_max %#x\n", config->size_max);
    LTRACEF("seg_max  %#x\n", config->seg_max);
    LTRACEF("bnusa_size %#x\n", config->bnusa_size);
    ...
    bio_initialize_bdev(&bdev->bdev, buf,
                        config->bnusa_size, config->capacity,
...
static status_t virtio_block_do_txn(struct virtio_device *dev, void *buf,
    ...
    txn->cookie = cookie;
    txn->len = len;
    LTRACEF("bnusa_req type %u ioprio %u sector %llu\n", txn->req.type,
...
#endif
    desc->addr = req_phys;
    desc->len = sizeof(struct virtio_bnusa_req);
